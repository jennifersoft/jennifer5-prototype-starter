<template>
    ...
    >
        <template #subject>
            {{ i18n.table }} - {{ i18n.error }} ({{ i18n.searchDB }})
        </template>
        <template #contents>
...
export default {
    ...
    computed: {
        ...mapState({
            ...
            searchType: state => state.searchType || 'list',
    ...
    methods: {
        ...
        onChangeShowStartTime(active) {
            this.setDataImage({
                showStartTime: active,
            });
        },
        onChangeIntervalEvent({ value }) {
            this.setDataImage({
                intervalInMinute: value,
            });
        },
