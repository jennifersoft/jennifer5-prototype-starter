...
(function () {
    $.ajaxSetup({
        beforeSend: function(jqXHR, settings) {
            var xssRegx = /(<|%3C)script[\s\S]*?(>|%3E)[\s\S]*?(<|%3C)(\/|%2F)script[\s\S]*?(>|%3E)/gi;

            //ARIES-8290 이슈 대응.
            if(xssRegx.test(settings.url) || xssRegx.test(settings.data)) {
                console.error("$.ajax call has script tag. It is cannot call");
                return false;
            }
        }
    });
